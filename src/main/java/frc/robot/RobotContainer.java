...
public class RobotContainer {
    ...
    private void configureBindings() {
        ...
        if (Robot.isReal()) {
            driverHID.axisGreaterThan(XboxController.Axis.kLeftTrigger.value, InputConstants.TRIGGER_THRESHOLD).onTrue(
                    Commands.runOnce(() -> beaterBar.setSpeed(BeaterBarConstants.INTAKE_SPEED)));
            driverHID.axisGreaterThan(XboxController.Axis.kLeftTrigger.value, InputConstants.TRIGGER_THRESHOLD)
                    .onFalse(Commands.runOnce(() -> beaterBar.setSpeed(0)));

            driverHID.axisGreaterThan(XboxController.Axis.kRightTrigger.value, InputConstants.TRIGGER_THRESHOLD).onTrue(
                    Commands.runOnce(() -> beaterBar.setSpeed(BeaterBarConstants.EJECT_SPEED)));
            driverHID.axisGreaterThan(XboxController.Axis.kRightTrigger.value, InputConstants.TRIGGER_THRESHOLD)
                    .onFalse(Commands.runOnce(() -> beaterBar.setSpeed(0)));
        }

        operatorHID.axisGreaterThan(XboxController.Axis.kLeftTrigger.value, InputConstants.TRIGGER_THRESHOLD).onTrue(
                Commands.sequence(
                        Commands.parallel(
                                new ElevatorSetHeight(elevator, ElevatorConstants.BARGE_HEIGHT),
                                new EndEffectorSetAngle(endEffector, elevator,
                                        EndEffectorConstants.ALGAE_INTAKE_ANGLE))));
